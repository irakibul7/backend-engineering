...
function PrefixEvidence({ stepIndex }: { stepIndex: number }) {
  return (
    <div className="prefix-workbench" aria-label="IPv4 prefix workbench">
      ...
      <div className="prefix-bit-grid" aria-label="Address bits: first 24 network bits, last 8 host bits">{(stepIndex >= 2 ? [203, 0, 113, 42] : [192, 0, 2, 10]).map(octet => octet.toString(2).padStart(8, "0")).join("").split("").map((bit, index) => <span key={index} className={index >= 24 ? "host-bit" : ""}>{bit}</span>)}</div>
...
export function NetworkingLab({ visualMode = "auto", embedded = false }: { visualMode?: VisualMode; embedded?: boolean }) {
  const Heading = embedded ? "h3" : "h1";
  const intersectionObserverAvailable = typeof window.IntersectionObserver === "function";
  const [state, setState] = useState(() => embedded ? selectScenario(createInitialEvidence(), "encapsulation") : createInitialEvidence());
  ...
  const current = evidenceStep(state);
  const effectivePlaying = isPlaying && pageVisible && inView && !reduceMotion && state.stepIndex < scenario.steps.length - 1;
  ...
  useEffect(() => { if (!embedded) document.documentElement.dataset.theme = dark ? "dark" : "light"; }, [dark, embedded]);
  ...
  return (
    <div className={`networking-lab ${embedded ? "is-embedded" : ""}`}>
      {!embedded ? <><a className="skip-link" href="#poc-main">Skip to networking proof</a>
      <Header dark={dark} onToggleTheme={() => setDark((currentDark) => !currentDark)} /></> : null}
      <div className="poc-shell">
        {!embedded ? <aside className="chapter-rail" aria-label="Chapter outline">
        ...
        <div id="poc-main" className="poc-main">
          ...
          <div className="journey-controls">
            ...
            <button type="button" className="primary-control" onClick={() => state.stepIndex === scenario.steps.length - 1 ? goToStep(0) : goNext()}>{state.stepIndex === scenario.steps.length - 1 ? "Replay" : "Continue"}<ChevronRight size={18} /></button>
          </div>
          <p className="control-hint">Choose a module, then use the controls or focus the step rail and press ← →.</p>
          {!embedded ? <details className="poc-metrics proof-metrics">
